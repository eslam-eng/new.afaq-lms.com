...
class FaqCategoryController extends Controller
{
    ...
    public function faq_cats_view()
    {
        $faqCategories = FaqCategory::orderBy('order','asc')->get();
        return view('admin.faqCategories.reorder',compact('faqCategories'));
    }
    public function sort_faq_cats(Request $request)

    {
        foreach ($request->order as $key => $order) {
            $faqCategories = FaqCategory::find($order['id'])->update(['order' => $order['order']]);
        }
