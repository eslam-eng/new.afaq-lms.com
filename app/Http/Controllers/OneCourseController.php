...
class OneCourseController extends Controller
{
    ...
    public function showNew($courses_id)
    {
        ...
        if ($oneCourse) {
            ...
            $oneCourse->load([
                ...
                'course_target', 'sponsors','organizers','course_sub_specialty'
            ]);
            ...
            $reservation_course = Enroll::reservation_number($oneCourse->id);

            $is_completed = is_null($oneCourse->seating_number)? False : ($reservation_course >= $oneCourse->seating_number);
        } else {
            return redirect()->back();
        }
        return view('onecourseindex', compact('oneCourse', 'related_courses', 'countries','reservation_course' , 'is_completed'));
    }
    ...
    public function quizeAnswar(Request $request)
    {
        try {
            ...
            return redirect()->route('one-course-content', ['course_id' => $quize->course_id, 'section_id' => $quize->section_id, 'locale' => app()->getLocale(),'lecture_id' => request('next_id')]);
        } catch (\Throwable $th) {
            //throw $th;
        }
    }
    ...
    public function content(Request $request)
    {
        ...
        $user_course = UsersCourse::where('course_id', $request->course_id)->where('user_id' , auth()->user()->id)->first();
        ...
        if ($oneCourse) {
            $oneCourse->load(['questionaire.questions.answars','course_target']);
        }

//        if (  $oneCourse->success_percentage >= $user_course->completion_percentage  )
//        {
//            $user_course->update([
...
//        }
    ...
    public function videoResult(Request $request)
    {
        ...
        $course_video_score = CourseVideoScore::where('user_id', auth()->user()->id)
            ...
            $data['score_percentage'] > 95 ? 100 : $data['score_percentage'];
    ...
    public function questionaire_result(Request $request)
    {
        ...
        $user_questionaire_before= CourseQuestionaireUserAnswar::where('course_questionaire_id',$request->course_questionaire_id)->where('course_id', $request->course_id )->where('user_id' , auth()->user()->id)->exists();
        ...
        if($user_questionaire_before){
            ...
            foreach ($request->answer as $question_id => $answer_name) {
                ...
                if (isset($answer_name[0])) {
                    if ($answer_name['type'] == 'text') {
                        $data['answar_text'] = $answer_name[0];
                        $data['course_questionaire_question_answar_id'] = null;
                    } elseif ($answer_name['type'] == 'true_false') {
                        $data['answar_text'] = (string)$answer_name[0];
                        $data['course_questionaire_question_answar_id'] = null;
                    } else {
                        $data['answar_text'] = null;
                        $data['course_questionaire_question_answar_id'] = $answer_name[0];
                    }
                    $data['user_id'] = auth()->user()->id;
