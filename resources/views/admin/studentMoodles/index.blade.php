...
@section('content')
@can('student_moodle_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.student-moodles.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.studentMoodle.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.studentMoodle.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-StudentMoodle">
                <thead>
                    <tr>
                        <th></th>
                        <th>
                ...
                <tbody>
                    @foreach($studentMoodles as $key => $studentMoodle)
                    <tr data-entry-id="{{ $studentMoodle->id }}">
                        <td></td>
                        <td>
                            {{ $studentMoodle->id ?? '' }}
                        </td>
                        ...
                        <td>
                            @can('student_moodle_show')

                            <a href="{{ route('admin.student-moodles.show', $studentMoodle->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('student_moodle_edit')

                            <a href="{{ route('admin.student-moodles.edit', $studentMoodle->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('student_moodle_delete')

                            <form action="{{ route('admin.student-moodles.destroy', $studentMoodle->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
