...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.cancelationPolicy.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.cancelation-policies.store') }}" enctype="multipart/form-data">
                @csrf
                    ...
                    <div data-repeater-list="cancelation_policy_values">
                        <div data-repeater-item class="row">
                            <div class="form-group col-5">
                                ...
                                <input class="form-control {{ $errors->has('days') ? 'is-invalid' : '' }}" type="text"
                                    name="days" id="days" value="{{ old('days', '') }}">
                                @if ($errors->has('days'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('days') }}
                                    </div>
                                @endif
                                <span class="help-block">{{ trans('cruds.cancelationPolicy.fields.days_helper') }}</span>
                            </div>
                            <div class="form-group col-5">
                                ...
                                @endif
                                <span class="help-block">{{ trans('cruds.cancelationPolicy.fields.amount_helper') }}</span>
                            </div>
                            <div class="col-2 form-group">
                                <label for="repeater-delete"></label>
                                <button id="repeater-delete" class="btn btn-danger form-control" data-repeater-delete type="button">{{ trans('global.delete') }} </button>
                            </div>
                        </div>
                    </div>

                    <button id="repeater-add" data-repeater-create class="btn btn-primary form-control col-2 my-2" type="button"> {{ trans('global.add') }} </button>
                </div>
