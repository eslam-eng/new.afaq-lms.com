...
@section('content')
@can('cancelation_policy_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.cancelation-policies.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.cancelationPolicy.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.list') }} {{ trans('cruds.cancelationPolicy.title_singular') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-CancelationPolicy">
                <thead>
                    <tr>
                        <th></th>
                        <th>
                ...
                <tbody>
                    @foreach($cancelationPolicies as $key => $cancelationPolicy)
                    <tr data-entry-id="{{ $cancelationPolicy->id }}">
                        <td></td>
                        <td>
                        ...
                        <td>
                            {{ $cancelationPolicy->course->name_ar ?? '' }}
                        </td>
                        <td>
                            {{ $cancelationPolicy->days ?? '' }}
                        </td>
                        <td>
                            {{ $cancelationPolicy->amount ?? '' }}
                        </td>
                        <td>
                            @can('cancelation_policy_show')
{{--                            <a class="btn btn-xs btn-primary" href="{{ route('admin.cancelation-policies.show', $cancelationPolicy->id) }}">--}}
{{--                                {{ trans('global.view') }}--}}
{{--                            </a>--}}
                                <a href="{{ route('admin.cancelation-policies.show', $cancelationPolicy->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('cancelation_policy_edit')
{{--                            <a class="btn btn-xs btn-info" href="{{ route('admin.cancelation-policies.edit', $cancelationPolicy->id) }}">--}}
{{--                                {{ trans('global.edit') }}--}}
{{--                            </a>--}}
                                    <a href="{{ route('admin.cancelation-policies.edit', $cancelationPolicy->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('cancelation_policy_delete')
{{--                            <form action="{{ route('admin.cancelation-policies.destroy', $cancelationPolicy->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">--}}
