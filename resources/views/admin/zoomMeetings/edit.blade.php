...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.zoomMeeting.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.zoom-meetings.update", [$zoomMeeting->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('meeting_type') ? 'is-invalid' : '' }}" name="meeting_type" id="meeting_type">
                    ...
                    @foreach(App\Models\ZoomMeeting::MEETING_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('meeting_type', $zoomMeeting->meeting_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                    @foreach($courses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $zoomMeeting->course->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('topic') ? 'is-invalid' : '' }}" type="text" name="topic" id="topic" value="{{ old('topic', $zoomMeeting->topic) }}" required>
                @if($errors->has('topic'))
                    <div class="invalid-feedback">
                        {{ $errors->first('topic') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="datetime-local" name="start_time" id="start_time" value="{{ old('start_time', $zoomMeeting->start_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="number" name="duration" id="duration" value="{{ old('duration', $zoomMeeting->duration) }}" step="1" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('host_video') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="host_video" id="host_video" value="1" {{ $zoomMeeting->host_video || old('host_video', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('participant_video') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="participant_video" id="participant_video" value="1" {{ $zoomMeeting->participant_video || old('participant_video', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\ZoomMeeting::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $zoomMeeting->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('audio') ? 'is-invalid' : '' }}" name="audio" id="audio">
                    ...
                    @foreach(App\Models\ZoomMeeting::AUDIO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('audio', $zoomMeeting->audio) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('audio'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('auto_recording') ? 'is-invalid' : '' }}" name="auto_recording" id="auto_recording">
                    ...
                    @foreach(App\Models\ZoomMeeting::AUTO_RECORDING_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('auto_recording', $zoomMeeting->auto_recording) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alternative_hosts') ? 'is-invalid' : '' }}" type="text" name="alternative_hosts" id="alternative_hosts" value="{{ old('alternative_hosts', $zoomMeeting->alternative_hosts) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('mute_upon_entry') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="mute_upon_entry" id="mute_upon_entry" value="1" {{ $zoomMeeting->mute_upon_entry || old('mute_upon_entry', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('watermark') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="watermark" id="watermark" value="1" {{ $zoomMeeting->watermark || old('watermark', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('waiting_room') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="waiting_room" id="waiting_room" value="1" {{ $zoomMeeting->waiting_room || old('waiting_room', 0) === 1 ? 'checked' : '' }}>
