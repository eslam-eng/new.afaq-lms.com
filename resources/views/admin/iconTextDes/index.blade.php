...
@section('content')
@can('icon_text_de_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.icon-text-des.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.iconTextDe.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.list') }} {{ trans('cruds.iconTextDe.title_singular') }} 
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-IconTextDe">
                <thead>
                    <tr>
<th></th>
                        <th>
                ...
                <tbody>
                    @foreach($iconTextDes as $key => $iconTextDe)
                    <tr data-entry-id="{{ $iconTextDe->id }}">
<td></td>
                        <td>
                            {{ $iconTextDe->id ?? '' }}
                        </td>
                        ...
                        <td>
                            @can('icon_text_de_show')

                                <a href="{{ route('admin.icon-text-des.show', $iconTextDe->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('icon_text_de_edit')

                                    <a href="{{ route('admin.icon-text-des.edit', $iconTextDe->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('icon_text_de_delete')

                                    <form action="{{ route('admin.icon-text-des.destroy', $iconTextDe->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
