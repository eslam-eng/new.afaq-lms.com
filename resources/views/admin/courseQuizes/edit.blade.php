...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.courseQuize.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.course-quizes.update", ['course_id' => $course_id,'id' => $courseQuize->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('exam_title') ? 'is-invalid' : '' }}" name="exam_title_id" id="exam_title_id" required>
                        @foreach($exam_titles as $id => $entry)
                            <option value="{{ $id }}" {{ (old('exam_title_id') ? old('exam_title_id') : $courseQuize->exam_title->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('title_en') ? 'is-invalid' : '' }}" type="text" name="title_en" id="title_en" value="{{ old('title_en', $courseQuize->title_en) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('title_ar') ? 'is-invalid' : '' }}" type="text" name="title_ar" id="title_ar" value="{{ old('title_ar', $courseQuize->title_ar) }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('description_en') ? 'is-invalid' : '' }}" name="description_en" id="description_en">{{ old('description_en', $courseQuize->description_en) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('description_ar') ? 'is-invalid' : '' }}" name="description_ar" id="description_ar" required>{{ old('description_ar', $courseQuize->description_ar) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('tips_guidelines') ? 'is-invalid' : '' }}" name="tips_guidelines" id="tips_guidelines">{{ old('tips_guidelines', $courseQuize->tips_guidelines) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('success_percentage') ? 'is-invalid' : '' }}" type="number" name="success_percentage" id="success_percentage" value="{{ old('success_percentage', $courseQuize->success_percentage) }}" step="0.01" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control datetime {{ $errors->has('start_at') ? 'is-invalid' : '' }}" type="datetime-local" name="start_at" id="start_at" value="{{ date('Y-m-d\TH:i', strtotime($courseQuize->start_at)) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control datetime {{ $errors->has('end_at') ? 'is-invalid' : '' }}" type="datetime-local" name="end_at" id="end_at" value="{{ old('end_at', $courseQuize->end_at) }}">
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\CourseQuize::STATUS_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $courseQuize->status) === (string) $key ? 'checked' : '' }} required>
