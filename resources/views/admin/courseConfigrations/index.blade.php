...
@section('content')
@can('course_configration_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.course-configrations.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.courseConfigration.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.list') }} {{ trans('cruds.courseConfigration.title_singular') }} 
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-CourseConfigration">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($courseConfigrations as $key => $courseConfigration)
                    <tr data-entry-id="{{ $courseConfigration->id }}">
                        <td>
                        ...
                        <td>
                            @can('course_configration_show')
                            <a href="{{ route('admin.course-configrations.show', $courseConfigration->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('course_configration_edit')
                            <a href="{{ route('admin.course-configrations.edit', $courseConfigration->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('course_configration_delete')
                            <form action="{{ route('admin.course-configrations.destroy', $courseConfigration->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
