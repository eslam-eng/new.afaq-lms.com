...
@section('content')
@can('sub_specialty_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.sub-specialties.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.subSpecialty.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.list') }} {{ trans('cruds.subSpecialty.title_singular') }} 
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-SubSpecialty">
                <thead>
                    <tr>
<th></th>
                        <th>
                ...
                <tbody>
                    ...
                    <tr data-entry-id="{{ $subSpecialty->id }}">
<td></td>
                        <td>
                            {{ $subSpecialty->id ?? '' }}
                        </td>
                        ...
                        <td>
                            @can('sub_specialty_show')

                                <a href="{{ route('admin.sub-specialties.show', $subSpecialty->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('sub_specialty_edit')

                                    <a href="{{ route('admin.sub-specialties.edit', $subSpecialty->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('sub_specialty_delete')

                                    <form action="{{ route('admin.sub-specialties.destroy', $subSpecialty->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
