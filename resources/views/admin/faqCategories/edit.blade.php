    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.faq-categories.update", [$faqCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('category_ar') ? 'is-invalid' : '' }}" type="text" name="category_ar" id="category_ar" value="{{ old('category_ar', $faqCategory->category_ar) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('category_en') ? 'is-invalid' : '' }}" type="text" name="category_en" id="category_en" value="{{ old('category_en', $faqCategory->category_en) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\FaqCategory::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $faqCategory->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
