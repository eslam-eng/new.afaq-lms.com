...
@section('content')
@can('point_type_value_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.point-type-values.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.pointTypeValue.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.pointTypeValue.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-PointTypeValue">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($pointTypeValues as $key => $pointTypeValue)
                        <tr data-entry-id="{{ $pointTypeValue->id }}">
                            <td>
                            ...
                            <td>
                                @can('point_type_value_show')
                                    <a href="{{ route('admin.point-type-values.show', $pointTypeValue->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                                ...
                                @can('point_type_value_edit')
                                        <a href="{{ route('admin.point-type-values.edit', $pointTypeValue->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                                ...
                                @can('point_type_value_delete')
                                      <form action="{{ route('admin.point-type-values.destroy', $pointTypeValue->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('point_type_value_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.point-type-values.massDestroy') }}",
