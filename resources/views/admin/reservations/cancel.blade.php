                                                    ...
                                                    if ($course_start_date) {
                                                        ...
                                                        if ($days >= 0) {
                                                            $cancel_policy = \App\Models\CancelationPolicy::where('course_id', $payment_detail->course_id)
                                                                ->whereHas('cancelValue', function ($values) use ($current_date) {
                                                                    if ($current_date > 21) {
                                                                        $values->where('days', '>=', 21);
                                                                    } elseif ($current_date <= 21 && $current_date > 3) {
                                                                        $values->where('days', '<=', 21)->where('days', '>', 3);
                                                                    } elseif ($current_date <= 3) {
                                                                        $values->where('days', '<=', 3);
                                                                    }
                                                                })
                                                                ->with([
                                                                    'cancelValue' => function ($values) use ($current_date) {
                                                                        if ($current_date > 21) {
                                                                            $values->where('days', '>=', 21);
                                                                        } elseif ($current_date <= 21 && $current_date > 3) {
                                                                            $values->where('days', '<=', 21)->where('days', '>', 3);
