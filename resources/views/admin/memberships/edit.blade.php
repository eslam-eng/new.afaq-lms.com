    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.memberships.update", [$membership->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('membership_type') ? 'is-invalid' : '' }}" name="membership_type_id" id="membership_type_id" required>
                    @foreach($membership_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('membership_type_id') ? old('membership_type_id') : $membership->membership_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('time_type') ? 'is-invalid' : '' }}" name="time_type" id="time_type" required>
                    ...
                    @foreach(App\Models\Membership::TIME_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('time_type', $membership->time_type) === (string) $key ? 'selected' : '' }}>{{ trans('cruds.membership.fields.'.$label)  }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $membership->price) }}" step="1" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $membership->link) }}" required>
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
