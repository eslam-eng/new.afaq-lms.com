<form action="" id="updateSectionLecture" class="update-normal-form" method="post" data-parsley-validate>
    ...
    <div class="row">
        <div class="form-group col-12">
            ...
            <select type="text" name="lecture_type" id="type-up" class="form-control type-up" required  data-parsley-required-message="{{ trans('global.required') }}">
                <option value="" disabled selected>{{ __('lms.select') }}</option>
                @foreach (['video', 'photo', 'file'] as $type)
        ...
        <div class="col-12 form-group my-auto">
            <label for=""></label>
            <button type="button" class="btn btn-success normal-update" lecture-id="{{ $lecture->id }}">
                {{ __('global.save') }}
            </button>
        </div>
    </div>
