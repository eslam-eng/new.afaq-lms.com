    ...
    @section('content')
        ...
        <div class="container-fluid" style="max-width: 93%;">
            <div class="row mt-5" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
                        ...
                        @if ($user_course)
                            <div class="lecture_progress_bar">
                                ...
                                <div class="progress_bar_top_info">
                                    ...
                                    <span class="bold sna_blue_color pr-2 pl-2">{{ $user_course->completion_percentage }}%</span>

                                    <div class="questionaire_button">
                                        @if (isset($user_course) && $oneCourse->success_percentage <= $user_course->completion_percentage && $oneCourse->questionaire)
                                            @if (auth()->check() &&
                ...
                <div class=" {{ app()->getLocale() == 'en' ? 'ltr' : 'rtl' }} course_content_right_sections_on_small_screen owl-carousel list-group">
                                ...
                                <div class="lecture_list_item_condition {{ $class }}">
                                    ...
                                    @switch($lecture->type)
                                        @case('video')
                                            <div class="video_thumbnail_container">
                                                <div class="video_thumbnail justify-content-center"
                                                    style="background-image: url('{{ asset('/nazil/imgs/video_icon.svg') }}')">
                                                    @if ($lecture->videoScore()->where('user_id', auth()->user()->id)->first())
                                                        <span
                                        ...
                                        @case('photo')
                                            ...
                                            <div class="video_thumbnail_container">
                                                <div class="video_thumbnail"
                                                    style="background-image: url({{ asset('/nazil/imgs/image_icon.svg') }})"> </div>
                                                <p>{{ $lecture->title }}</p>
                                            </div>
                                        @break
                                        ...
                                        @case('file')
                                            <div class="video_thumbnail_container ">
                                                <div class="video_thumbnail"
                                                    style="background-image: urk({{ asset('/nazil/imgs/file-regular.svg') }})"></div>
                                                <p>{{ $lecture->title }}</p>
                                            </div>
                                        @break
                ...
                <div class="col-12 order-xl-2 order-1" id="right-notes">
                    <h4 class="course_title on_large_screen">
                        {{ app()->getLocale() == 'en' ? $oneCourse->name_en : $oneCourse->name_ar }}
                    </h4>
                    <div class="progress_line_parent on_large_screen">
                        ...
                        @if ($user_course)
                            <div class="lecture_progress_bar">
                                <span class="bold">{{ __('global.completion_percentage') }}</span>
                                <span class="bold sna_blue_color">{{ $user_course->completion_percentage }}%</span>

                                <div class="progress_white_line">
                                    <span class="blue-line" style="width: {{ $user_course->completion_percentage ?? 0 }}%;"></span>
                                </div>
                            </div>
                        @endif
                    </div>
                                    ...
                                    <div class="accordion-item">
                                        <h2 class="accordion-header {{ $loop->index == 0 ? 'show' : 'in' }}"
                                            ...
                                            <button class="accordion-button {{ $loop->index == 0 ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse"
                                                data-bs-target="#coll-{{ $section->id }}" aria-expanded="{{ $loop->index == 0 ? 'true' : 'false' }}"
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach ($oneCourse->sections as $key => $sect)
                                    @foreach ($sect->lectures as $keyLec => $lect)
                                        <div class="tab-pane fade  {{ $lect->id == $lecture_id ? 'active show' : '' }} "
    ...
    {{--                                                        ({{ $question['type'] }})--}}
                                                ...
                                                <ul class="choices">
                                                    ...
                                                    @elseif($question['type'] == 'text')
                                                        ...
                                                        <input type="hidden" name="questions[{{ $qkey }}][type]"
                                                            value="{{ $question['type'] }}">
                                                        <li><label class="d-flex">
                                                                <textarea id="answer_id{{ $qkey }}" class="col-11"
                                                                        name="questions[{{ $qkey }}][answer_id]"></textarea></label>
                                                        </li>
                                    ...
                                    <div class="form-group m-2 pt-2 d-flex justify-content-center" style="border: none; background-color: #845097;">
    ...
    <script>
        function backLecture(lectureId) {
            if($('.course_content_right_sections .accordion-item').length){
                ...
                var prevA = $('.course_content_right_sections a.list-group-item.active').prev()
                    ...
                    parentAccordion = $('.course_content_right_sections a.list-group-item.active').parents('.accordion-item'),
                ...
                if (prevA.length > 0 && prevDiv.length > 0 && !$(prevA).hasClass('disabled')) {
                    // console.log(prevA)
                    $('.course_content_right_sections a.list-group-item.active').attr('aria-selected', 'false').removeClass('active')
                ...
                if(parentAccordionPrevChild.length && !prevA.length > 0){
                    ...
                    $('.course_content_right_sections .accordion-item').children('.accordion-header').children("button[aria-expanded = 'true']").addClass('collapsed')
                    $('.course_content_right_sections .accordion-item').children('.accordion-header').children("button[aria-expanded = 'true']").attr('aria-expanded', 'false')
                    $('.course_content_right_sections .accordion-item').children('.accordion-header.show').removeClass('show')
                    $('.course_content_right_sections .accordion-item').children('.accordion-collapse.show').removeClass('show')

                    parentAccordionPrev.children('.accordion-header').children("button[aria-expanded = 'false']").removeClass('collapsed')
                    parentAccordionPrev.children('.accordion-header').children("button[aria-expanded = 'false']").attr('aria-expanded', 'true')
            ...
            if($('.course_content_right_sections_on_small_screen').length){
                var prevA = $('.course_content_right_sections_on_small_screen a.list-group-item.active').parents('.owl-item').prev().find('a.list-group-item')
            ...
            //         // parentAccordionPrevChild = parentAccordionPrev.find('.list-group a.list-group-item:not(.disabled):last-of-type');
                ...
                if (prevA.length > 0 && !$(prevA).hasClass('disabled')) {
                    $('.course_content_right_sections_on_small_screen a.list-group-item.active').attr('aria-selected', 'false').removeClass('active')
                    prevA.addClass('active')
                    prevA.attr('aria-selected', 'true')
        ...
        function nextLecture(lectureId) {
            ...
            if($('.course_content_right_sections_on_small_screen').length){
                var prevA = $('.course_content_right_sections_on_small_screen a.list-group-item.active').parents('.owl-item').next().find('a.list-group-item')
            //         // prevDiv = $('div.tab-pane.active.show').next(),
            //         // parentAccordion = $('.course_content_right_sections_on_small_screen a.list-group-item.active').parents('.accordion-item'),
            //         // parentAccordionPrev = parentAccordion.next(),
            //         // parentAccordionPrevChild = parentAccordionPrev.find('.list-group a.list-group-item:not(.disabled):last-of-type');
    ...
    @section('scripts')
        ...
        <script>
            ...
            $('.course_content_right_sections .accordion-item').click(function(){
                $(this).siblings('.accordion-item').children('.accordion-header').children("button[aria-expanded = 'true']").addClass('collapsed')
                $(this).siblings('.accordion-item').children('.accordion-header').children("button[aria-expanded = 'true']").attr('aria-expanded', 'false')
