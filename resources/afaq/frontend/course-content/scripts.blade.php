...
<script>
    ...
    function setVideoNotes(lecture_id) {
        ...
        if (notes.course_notes.length) {
            $('.left-notes').addClass('col-3');
            $('.right-notes').addClass('col-9');
            $('.right-notes').removeClass('col-12');
            $('.left-notes').show();

            $('.render-notes').empty();
            $.each(notes.course_notes, function(key, value) {
                $('.render-notes').append(`
                        <div class="content-note-data notes_section notes-btn" lectureId="${lecture_id}" time="${value.in_time}">
                                <div class="note-img">
                                    <img src="{{ asset('/nazil/imgs/course_image_100-272x161.jpg') }}" alt="">
                                    <div class="small-playe-img">
                                        <img src="{{ asset('afaq/imgs/caret-right-solid.png') }}" alt="">
                                    </div>
                                    <span>${value.in_time}</span>
                                </div>
                                <p>
                                    ${ value.note ?? '' }
                                </p>
            ...
            $('.left-notes').hide()
            $('.right-notes').removeClass('col-9');
            $('.left-notes').removeClass('col-3');
            $('.right-notes').addClass('col-12')
        ...
        $(".video-js").each(function(videoIndex) {
            ...
            window[`player_${videoId}`].ready(function() {
            ...
            window[`player_${lecture_id}`].play()

    }

    function removeNotes(lecture_id) {
        $('.left-notes').hide()
        $('.right-notes').addClass('col-12')
        $('.right-notes').removeClass('col-9');
        $('.render-notes').empty();
