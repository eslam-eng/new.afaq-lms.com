...
{{--{{dd($payment)}}--}}
    <div class="pdf-container">
        ...
        <div class="header-content d-flex flex-row">
            <div class="barcode-image mb-5">
{{--                {{$payment->qr_image}}--}}
                <img alt="" src="{{$payment->qr_image}}">
            </div>
...
{{--                    @foreach ($payment->payment_details as $payment_detail)--}}
                        <tr>
                            <td>{{ $payment->id }}</td>
                            <td>{{ $payment->package->package_name_ar ?? '' }}</td>
{{--                            <td>{{ $payment->price ?? ''}}</td>--}}
{{--                            <td>{{ $payment->offer ?? ''}}</td>--}}
                            <td>{{ $payment->price }}</td>
                            <td>1</td>
...
{{--                    @foreach ($payment->payment_details as $payment_detail)--}}
                    <tr>
                        <td>{{ $payment->id }}</td>
                        <td>{{ $payment->package->package_name_en ?? '' }}</td>
                        {{--                            <td>{{ $payment->price ?? ''}}</td>--}}
                        {{--                            <td>{{ $payment->offer ?? ''}}</td>--}}
                        <td>{{ $payment->price }}</td>
                        <td>1</td>
        ...
        <div class="last-section d-flex flex-column">
            ...
            <div class="total-table d-flex flex-row">
                <table class="table-left">
                    @php
                        $total_without_tax = (float)($payment->price / (1+0.15));
                    @endphp
                    <tr>
                        <th>Total</th>
                    ...
                    <tr>
                        ...
                        <td>{{ number_format((float)((float)$payment->price - $total_without_tax), 2, '.', '') }}</td>
                    </tr>
                    <tr>
                        <th>Final Total</th>
                        <td>{{ $payment->price }} SAR</td>
                    </tr>
                </table>
