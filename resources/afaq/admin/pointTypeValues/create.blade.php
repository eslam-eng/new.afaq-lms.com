...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pointTypeValue.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.point-type-values.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('point_type') ? 'is-invalid' : '' }}" name="point_type_id" id="point_type_id" required>
                    @foreach($point_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('point_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('give_point') ? 'is-invalid' : '' }}" type="number" name="give_point" id="give_point" value="{{ old('give_point', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('get_point') ? 'is-invalid' : '' }}" type="number" name="get_point" id="get_point" value="{{ old('get_point', '') }}" step="1">
