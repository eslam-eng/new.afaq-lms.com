        ...
        <form method="POST" action="{{ route("admin.course-invoices.update", [$courseInvoice->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.courseInvoice.fields.users') }}</label>
                    <input type="text" name="{{ $courseInvoice->user ? $courseInvoice->user->email : '' }}" class="form-control" id="users_input">
                    <input type="hidden" value="{{ $courseInvoice->user_id }}" name="user_id" id="user_id">
                @if($errors->has('user'))
