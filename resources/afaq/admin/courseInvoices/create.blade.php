        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.course-invoices.store', ['locale' => app()->getLocale()]) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="" class="form-control" id="users_input">
...
{{--                    </div>--}}
                    <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}"
                        name="courses_ids[]" id="course_id"  required onchange="setPrice(this)">
                        @foreach ($courses as $course)
                            <option price="{{$course->today_price}}" value="{{ $course->id }}" {{ old('course_id') == $course->id ? 'selected' : '' }} >
                                {{ $course->name }}</option>
                        @endforeach
                    </select>
                        ...
                        @foreach ($methods as $id => $entry)
                            ...
                            <option value="{{ $entry->id }}"
                                ...
                                free="{{ $entry->name_en == 'free' || $entry->name_en == 'Free' ? true : false }}"
                                    wallet="{{ $entry->name_en == 'wallet' || $entry->name_en == 'Wallet' ? true : false }}"
                            >
                                {{ $entry->name_en }}
                            </option>
                        @endforeach
                ...
                <div class="form-group ">
                    <label for="admin_invoice_note">{{ trans('cruds.courseInvoice.fields.admin_invoice_note') }}</label>
                    <input class="form-control {{ $errors->has('admin_invoice_note') ? 'is-invalid' : '' }}" type="text"
                           name="admin_invoice_note" id="admin_invoice_note" value="{{ old('admin_invoice_note', '') }}">
                    @if ($errors->has('admin_invoice_note'))
                        <div class="invalid-feedback">
                            {{ $errors->first('admin_invoice_note') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.courseInvoice.fields.admin_invoice_note_helper') }}</span>
                </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function change_payment_method() {
            ...
            let wallet = $('#payment_method_id').find('option:selected').attr('wallet');
            // let wallet = $('#payment_method_id').val('wallet');
            if (wallet) {
                ...
                var user_id = $('#user_id').val();
                    ...
                    $.ajax({
                        type: 'GET',
                        url: '/admin/users/getUserWallet?amount='+amount+'&id='+user_id,
                        error: function(err) {
                            console.log(err);
                        },
                        success: function(data) {
                                if(!data){
                                    alert('You Do Not Have Enough Balance In Your Wallet ')
                                }
    ...
    <script>
        $(function() {
            $("#users_input").autocomplete({
                ...
                select: function(event, ui) {
                    console.log(ui);
                    $("#user_id").val(ui.item.value); // <== KO
                    $(' #users_input').val(ui.item.label); // <== OK
                    return false;
                },
        ...
        function setPrice(el){
            $('#amount').val(el.options[el.selectedIndex].getAttribute('price'));
