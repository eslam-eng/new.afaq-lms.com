    <!-- Add Lecture  -->
    <div class="modal fade" id="zoomLectureModal" tabindex="-1" role="dialog" aria-labelledby="zoomLectureModalTitle"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('admin.zoom-meetings.store') }}" enctype="multipart/form-data"
                        class="d-flex flex-wrap create-zoom-form" data-parsley-validate>
                        @csrf
                        ...
                        <div class="form-group col-12" style="display: grid;">
                            ...
                            <select class="form-control select2 {{ $errors->has('accessing') ? 'is-invalid' : '' }}"
                                name="accessing" id="accessing" required
                        ...
                        <div class="form-group col-6">
                            ...
                            <select class="form-control {{ $errors->has('meeting_type') ? 'is-invalid' : '' }}"
                                name="meeting_type" id="meeting_type">
                                ...
                                @foreach (App\Models\ZoomMeeting::MEETING_TYPE_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('meeting_type', '') === (string) $key ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-6">
                            <label class="required"
                                for="topic">{{ trans('cruds.zoomMeeting.fields.topic') }}</label>
                            <input class="form-control {{ $errors->has('topic') ? 'is-invalid' : '' }}" type="text"
                                name="topic" id="topic" required value="{{ old('topic', '') }}">
                            @if ($errors->has('topic'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('topic') }}
                                </div>
                        ...
                        <div class="form-group col-6">
                            <label class="required"
                                for="start_time">{{ trans('cruds.zoomMeeting.fields.start_time') }}</label>
                        ...
                        <div class="form-group col-6">
                            <label class="required"
                                for="duration">{{ trans('cruds.zoomMeeting.fields.duration') }}</label>
                            <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}"
                                type="number" name="duration" id="duration" value="{{ old('duration', '') }}"
                                step="1" required data-parsley-required-message="{{ trans('global.required') }}">
                        ...
                        <div class="form-group col-6">
                            <div class="form-check {{ $errors->has('host_video') ? 'is-invalid' : '' }}">
                                ...
                                <input class="form-check-input" type="checkbox" name="host_video" id="host_video"
                                    value="1" {{ old('host_video', 0) == 1 ? 'checked' : '' }}>
                                <label class="form-check-label"
                                    for="host_video">{{ trans('cruds.zoomMeeting.fields.host_video') }}</label>
                            </div>
                        ...
                        <div class="form-group col-6">
                            <div class="form-check {{ $errors->has('participant_video') ? 'is-invalid' : '' }}">
                                ...
                                <input class="form-check-input" type="checkbox" name="participant_video"
                                    id="participant_video" value="1"
                                    {{ old('participant_video', 0) == 1 ? 'checked' : '' }}>
                                <label class="form-check-label"
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.zoomMeeting.fields.participant_video_helper') }}</span>
                        </div>
                        <div class="form-group col-6">
                        ...
                        <div class="form-group col-6">
                            ...
                            <select class="form-control {{ $errors->has('auto_recording') ? 'is-invalid' : '' }}"
                                name="auto_recording" id="auto_recording">
                                ...
                                @foreach (App\Models\ZoomMeeting::AUTO_RECORDING_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('auto_recording', 'cloud') === (string) $key ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.zoomMeeting.fields.auto_recording_helper') }}</span>
                        </div>
                        <div class="form-group col-6">
                            <label
                                for="alternative_hosts">{{ trans('cruds.zoomMeeting.fields.alternative_hosts') }}</label>
                            <input class="form-control {{ $errors->has('alternative_hosts') ? 'is-invalid' : '' }}"
                                type="text" name="alternative_hosts" id="alternative_hosts"
                                value="{{ old('alternative_hosts', '') }}">
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.zoomMeeting.fields.alternative_hosts_helper') }}</span>
                        </div>
                        <div class="form-group col-6">
                            <div class="form-check {{ $errors->has('mute_upon_entry') ? 'is-invalid' : '' }}">
                                <input type="hidden" name="mute_upon_entry" value="0">
                                <input class="form-check-input" type="checkbox" name="mute_upon_entry"
                                    id="mute_upon_entry" value="1"
                                    {{ old('mute_upon_entry', 0) == 1 ? 'checked' : '' }}>
                                <label class="form-check-label"
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.zoomMeeting.fields.mute_upon_entry_helper') }}</span>
                        </div>
                        <div class="form-group col-6">
                            <div class="form-check {{ $errors->has('watermark') ? 'is-invalid' : '' }}">
                                ...
                                <input class="form-check-input" type="checkbox" name="watermark" id="watermark"
                                    value="1" {{ old('watermark', 0) == 1 ? 'checked' : '' }}>
                                <label class="form-check-label"
                                    for="watermark">{{ trans('cruds.zoomMeeting.fields.watermark') }}</label>
                            </div>
                        ...
                        <div class="form-group col-6">
                            <div class="form-check {{ $errors->has('waiting_room') ? 'is-invalid' : '' }}">
                                ...
                                <input class="form-check-input" type="checkbox" name="waiting_room"
                                    id="waiting_room" value="1"
                                    {{ old('waiting_room', 0) == 1 || old('waiting_room') === null ? 'checked' : '' }}>
                                <label class="form-check-label"
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.zoomMeeting.fields.waiting_room_helper') }}</span>
                        </div>
                        <div class="form-group col-12">
                            <label for="agenda">{{ trans('cruds.zoomMeeting.fields.agenda') }}</label>
                            <textarea class="form-control ckeditor {{ $errors->has('agenda') ? 'is-invalid' : '' }}" rows="3"
                                name="agenda" id="agenda">{!! old('agenda') !!}</textarea>
                        ...
                        <div class="form-group col-6">
                            <button class="btn btn-success zoom-submit" modal-div-id="zoomLectureModal"
                                type="button">
                                {{ trans('global.save') }}
                            </button>
                        </div>
    ...
    @push('footer-scripts')
        <script>
            $(document).on('click', '.zoom-button', function() {
                var section_id = $(this).attr('zoom-section-id');
                var course_id = $(this).attr('zoom-course-id');
                $('.zoom-submit').attr('zoom-section-id', section_id);
                $('.zoom-submit').attr('zoom-course-id', course_id);
            });
            $(document).on('click', '.zoom-submit', function() {
                if (!$('.create-zoom-form').parsley().isValid()) {
                    $('.create-zoom-form').parsley().validate();
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#zoomLectureModal').modal('toggle');
                        // $(`.card-opener-${section_id}`).click();
                        $('.create-zoom-form').trigger("reset");
