   ...
   <script src="https://cdn.tiny.cloud/1/xdtvumic5spz8s20xfmw404uqqibaqka8var52dipy3hxwag/tinymce/7/tinymce.min.js" referrerpolicy="origin"></script>
        ...
        $(".number-tab-steps").steps({
            ...
            onFinished: function(event, currentIndex) {
                ...
                if ($(this).parsley().isValid(`step-${currentIndex + 1}`)) {
                    var frm = $('#course_form');
                    var url = "{{ route('admin.courses.store') }}";
                    ...
                    if ($(this).parsley().isValid(`step-${currentIndex + 1}`)) {
                        ...
                        $(`[data-parsley-group="step-${currentIndex + 1}"]`).each(function(i, input) {
                            if (input.type == 'checkbox') {
                                ...
                                var values = [];
                                $(`#${input.id} :selected`).each(function(i, selected) {
                                    formData.append(input.name.replace('[]', `[${i}]`), $(
                                        selected).val());
                                });
                            } else {
                                formData.append(input.name, input.value);
    ...
    <script>
        ...
        function getSubClassifications(value) {
            ...
            if (sub_classifications.length) {
                ...
                $.each(sub_classifications, function(index, sub) {
                    $('#course_sub_classification_id').append(`<option value="${sub.id}">${sub.name}</option>`)
                });
    ...
    <script>
        $("#has_special_policy").change(function() {
            if ($(this).is(':checked')) {
                $(".mycheckboxdiv").show();
                // $('#policy_en').prop('required', true);
                // $('#policy_ar').prop('required', true);
            } else {
                $(".mycheckboxdiv").hide();
                // $('#policy_en').prop('required', false);
                // $('#policy_ar').prop('required', true);
            }
        });
    </script>
    ...
    <script>
        function getCities(countryId, selected_city = city_id) {
            ...
            if (targetCountry.cities) {
                if (targetCountry.cities.length) {
                    $('#city_id').empty();
                    $('#city_id').append(`<option value="" disabled></option>`);
                    $.each(targetCountry.cities, function(key, city) {
                        $('#city_id').append(
                            `<option value="${city.id}" ${(selected_city && selected_city == city.id) ? 'selected' : ''}>${city.title}</option>`
                        );
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(key, sub_specialty) {
                            ...
                            $('#course_sub_specialty_id').append(
                                `<option value="${sub_specialty.id}">${sub_specialty.name}</option>`
                            );
