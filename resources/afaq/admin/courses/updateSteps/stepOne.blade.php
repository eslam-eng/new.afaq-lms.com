    ...
    <div class="row col-12 ">
                    ...
                    <select class="form-control select2" name="course_organizers[]" multiple
                        ...
                        @foreach ($lookups as $lookup)
                            @if ($lookup->type->slug == 'course_organizers')
                                <option value="{{ $lookup->id }}"  {{( in_array($lookup->id,$selected_course_organizers)) ? 'selected' : ''}}>
                                    {{ $lookup->title }}</option>
                            @endif
                        @endforeach
                    </select>
        ...
        <div class="row col-12">
            ...
            <div class="row col-12  place-hidden-section" style="display:none !important;">
                <div class="form-group col-6">
                    <label class="label-waves-effect" style="position:relative; top:0;" for="attendance_design_id ">{{ trans('cruds.attendanceDesign.title_singular') }}</label>
                    <select class="form-control select2 {{ $errors->has('attendanceDesign') ? 'is-invalid' : '' }}"  name="attendance_design_id[]" data-parsley-group="step-1" id="attendance_design_id">
                        @foreach($attendance_designs as $attendance_design)
                            <option value="{{ $attendance_design->id }}" {{ in_array($attendance_design->id , count($course_attendes_selected_array) > 0 ? $course_attendes_selected_array : []) ? 'selected' : '' }}>{{ $attendance_design->name }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('attendanceDesign'))
                        <div class="invalid-feedback">
                            {{ $errors->first('attendanceDesign') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.attendanceDesign.fields.attendanceDesign_helper') }}</span>
                </div>
                <div class="form-group col-6">
                    <label style="position:relative; top:0;" for="attend_type " >{{ trans('cruds.course.fields.attend_type') }}</label>
                    <select class="form-control {{ $errors->has('attend_type') ? 'is-invalid' : '' }}"
                            data-parsley-errors-container="#attend_type_error" data-parsley-group="step-1" data-parsley-required-message="{{ trans('global.required') }}"  name="attend_type" id="attend_type">
                        ...
                        @foreach(App\Models\Course::ATTEND_TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('attend_type', $course->attend_type) === (string) $key ? 'selected' : '' }}>{{ trans('cruds.course.fields.'.$label)  }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-6">
                    ...
                    <input class="form-control {{ $errors->has('detailed_address') ? 'is-invalid' : '' }}" type="text" name="detailed_address"
                           id="detailed_address"  value="{{$course->detailed_address}}"  data-parsley-errors-container="#course_detailed_address" data-parsley-group="step-1" data-parsley-required-message="{{ trans('global.required') }}">
