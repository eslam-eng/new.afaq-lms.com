...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.system-emails.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">Type (Choose if this email content will be for a Course, a Exam or General )</label>
            ...
            <div class="form-group" id="programmes_field">
                <label class="required">Exam</label>
                <select class="form-control select2 {{ $errors->has('exam_id') ? 'is-invalid' : '' }}" name="exam_id" id="exam_id">
                    @foreach($exams as $id => $entry)
                        <option value="{{ $id }}" {{ (old('exam_id') ? old('exam_id') : $system_email->exam_id ?? '') == $entry ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" id="lectures_field">
                ...
                <select class="form-control select2 {{ $errors->has('course_id') ? 'is-invalid' : '' }}" name="course_id" id="course_id">
                    @foreach($courses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $system_email->course_id ?? '') == $entry ? 'selected' : '' }}>{{ $entry}}</option>
                    @endforeach
                </select>
  ...
  // }
    ...
    $('#search_emails').click(function(){
        ...
        $.ajax({
            ...
            data:'mail_type='+mail_type+'&name_id= '+name_id+'&exam_id='+exam_id+'&course_id='+course_id,
            beforeSend: function(){
                $("#loader").show();
                $("#return_url").html(" ");
                // $("#return_url").hide();
