...
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.question-banks.create') }}">
...
<div class="card">
    <div class="card-header">
        @if(App::getLocale() == 'en')
       <bdi> {{ trans('cruds.questionBank.title_singular') }} {{ trans('global.list') }}</bdi>
        @else
       <bdi> {{ trans('global.list') }} {{ trans('cruds.questionBank.title_singular') }} </bdi>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-QuestionBank">
                <thead>
                    <tr>
<th></th>
                        <th>
                ...
                <tbody>
                    @foreach($questionBanks as $key => $questionBank)
                    <tr data-entry-id="{{ $questionBank->id }}">
<td></td>
                        <td>
                            {{ $questionBank->id ?? '' }}
                        </td>
                        ...
                        <td>
                            @can('question_bank_show')
                                ...
                                <a href="{{ route('admin.question-banks.show', $questionBank->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('question_bank_edit')
                                    ...
                                    <a href="{{ route('admin.question-banks.edit', $questionBank->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('question_bank_delete')
                                    ...
                                    <form action="{{ route('admin.question-banks.destroy', $questionBank->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
