...
@section('content')
@can('business_special_request_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.business-special-requests.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.businessSpecialRequest.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.businessSpecialRequest.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-BusinessSpecialRequest">
                <thead>
                    <tr>
                        <th width="10">
...
{{--                        </th>--}}
                ...
                <tbody>
                    @foreach($businessSpecialRequests as $key => $businessSpecialRequest)
                        <tr data-entry-id="{{ $businessSpecialRequest->id }}">
                            <td>
...
{{--                            </td>--}}
                            <td>
                                @can('business_special_request_show')
                                    <a href="{{ route('admin.business-special-requests.show', $businessSpecialRequest->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                                ...
                                @can('business_special_request_edit')
                                        <a href="{{ route('admin.business-special-requests.edit', $businessSpecialRequest->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                                ...
                                @can('business_special_request_delete')

                                        <form action="{{ route('admin.business-special-requests.destroy', $businessSpecialRequest->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('business_special_request_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.business-special-requests.massDestroy') }}",
...
@endcan
  ...
  // let table = $('.datatable-BusinessSpecialRequest:not(.ajaxTable)').DataTable({ buttons: dtButtons })
