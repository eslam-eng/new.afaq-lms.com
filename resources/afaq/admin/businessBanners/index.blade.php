...
@section('content')
@can('business_banner_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.business-banners.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.businessBanner.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.businessBanner.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-BusinessBanner">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($businessBanners as $key => $businessBanner)
                        <tr data-entry-id="{{ $businessBanner->id }}">
                            <td>
                            ...
                            <td>
                                @can('business_banner_show')
                                    <a href="{{ route('admin.business-banners.show', $businessBanner->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                                ...
                                @can('business_banner_edit')
                                        <a href="{{ route('admin.business-banners.edit', $businessBanner->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                                ...
                                @can('business_banner_delete')
                                    <form action="{{ route('admin.business-banners.destroy', $businessBanner->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('business_banner_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.business-banners.massDestroy') }}",
...
@endcan
  ...
  // let table = $('.datatable-BusinessBanner:not(.ajaxTable)').DataTable({ buttons: dtButtons })
