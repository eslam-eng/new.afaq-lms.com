...
@section('content')
@can('business_event_type_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.business-event-types.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.businessEventType.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.businessEventType.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-BusinessEventType">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($businessEventTypes as $key => $businessEventType)
                        <tr data-entry-id="{{ $businessEventType->id }}">
                            <td>
                            ...
                            <td>
                                @can('business_event_type_show')
                                    <a href="{{ route('admin.business-event-types.show', $businessEventType->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                                ...
                                @can('business_event_type_edit')

                                        <a href="{{ route('admin.business-event-types.edit', $businessEventType->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                                ...
                                @can('business_event_type_delete')
                                        <form action="{{ route('admin.business-event-types.destroy', $businessEventType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('business_event_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.business-event-types.massDestroy') }}",
...
@endcan
  ...
  // let table = $('.datatable-BusinessEventType:not(.ajaxTable)').DataTable({ buttons: dtButtons })
