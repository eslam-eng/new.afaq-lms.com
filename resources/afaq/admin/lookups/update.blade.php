...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{$lookup_type->title }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.lookups.update", ['id' => $lookup->id,'type_slug'=> $type_slug]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label for="name_en">{{ trans('cruds.lookup.fields.name_en') }}</label>
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text" name="title_en" id="name_en" value="{{ old('name_en', $lookup->title_en) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_ar') ? 'is-invalid' : '' }}" type="text" name="title_ar" id="name_ar" value="{{ old('name_ar', $lookup->title_ar) }}" required>
                ...
                @if ($type_slug == 'course_classifications')
                    <div class="form-group">
                        ...
                        <select style="width: 100%;"
                            ...
                            <option value=""></option>
                                <option value="{{ $parent_lookup->id }}"
                                    {{ ($lookup->parent_id == $parent_lookup->id) ? 'selected' : '' }}>{{ $parent_lookup->title }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group wrap-custom-file">
                    ...
                    <input type="file" value="{{$lookup->image ? $lookup->image : null}}" name="image" id="image1" accept=".gif, .jpg, .png" />
                    <label for="image1">
                        <span>Image </span>
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\Lookup::STATUS_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $lookup->status) == (string) $key ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('show_in_homepage') ? 'is-invalid' : '' }}" name="show_in_homepage" id="show_in_homepage">
                        ...
                        @foreach(App\Models\Lookup::SHOW_IN_HOMEPAGE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('show_in_homepage', $lookup->show_in_homepage) === (string) $key ? 'selected' : '' }}>{{ trans('cruds.testimonial.fields.'.$label)  }}</option>
                        @endforeach
                    </select>
        ...
        Dropzone.options.imageDropzone = {
            ...
            init: function () {
                ...
                setTimeout(function () {
                    var t = tinyMCE.get(0);
                    var edObj = t.dom.getRoot();
                    t.dom.setStyle(edObj, 'background-image', `url(/storage/${file.id}/${file.file_name})`);
                    t.dom.setStyle(edObj, 'background-position', `center`);
                    t.dom.setStyle(edObj, 'background-size', `100% 100%`);
                    t.dom.setStyle(edObj, 'background-repeat', `no-repeat`);
                    t.dom.setStyle(edObj, 'height', `700px`);
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('input[type="file"]').each(function() {
                ...
                $label.addClass('file-ok').css('background-image', "url({{($lookup->image_url)}} )");
