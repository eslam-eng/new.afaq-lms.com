            ...
            <div class="form-group">
                ...
                <table class="table table-bordered table-striped">
                    <tbody>
                        ...
                        <tr>
                            <th>
                                {{ trans('cruds.courseInvoice.fields.admin_invoice_note') }}
                            </th>
                            <td>

                                {{ $reservation->payment_invoices->admin_invoice_note ?? '' }}
                            </td>
                        </tr>
                        <tr>
                            <th>
                            ...
                            <td>
                                {{-- {{ $reservation->status_response ? (json_decode($reservation->status_response)->status == 'Failed' ? __('afaq.failed') : json_decode($reservation->status_response)->status ?? '') : $reservation->status }} --}}
                                @php
                                    $response = json_decode($reservation->status_response);
                                @endphp
                                {{-- {{ $reservation->status_response ? (isset($response->response) ? (isset(json_decode($response->response)->description) ? json_decode($response->response)->description :'') : '') : '' }} --}}
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane active" role="tabpanel" id="order_payment">
                @includeIf('admin.reservations.relationships.payment_details', [
                    'reservation_payments' => $reservation->payment_details,
                ])
            </div>
