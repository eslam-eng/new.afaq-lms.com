    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.testimonials.update", [$testimonial->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.testimonial.fields.title_en_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="title_en">{{ trans('cruds.testimonial.fields.title_en') }}</label>
                <input class="form-control {{ $errors->has('title_en') ? 'is-invalid' : '' }}" type="text" name="title_en" id="title_en" value="{{ old('title_en', $testimonial->title_en) }}" required>
            ...
            <div class="form-group">
                <label class="required" for="image">{{ trans('cruds.testimonial.fields.image') }}<small>1009 X 312 px</small></label>
            ...
            <div class="form-group">
                <label class="required" for="description_en">{{ trans('cruds.testimonial.fields.description_en') }}</label>
                <textarea class="form-control {{ $errors->has('description_en') ? 'is-invalid' : '' }}" name="description_en" id="description_en" required>{{ old('description_en', $testimonial->description_en) }}</textarea>
                @if($errors->has('description_en'))
                    <div class="invalid-feedback">
                        {{ $errors->first('description_en') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.testimonial.fields.description_en_helper') }}</span>
            </div>
            <div class="form-group">
                <label>{{ trans('cruds.testimonial.fields.status') }}</label>
                @foreach(App\Models\Testimonial::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $testimonial->status) === (string) $key ? 'checked' : '' }}>
