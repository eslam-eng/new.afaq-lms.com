...
@section('content')
@can('ticket_comment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.ticket-comments.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.ticketComment.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.ticketComment.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-TicketComment">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($ticketComments as $key => $ticketComment)
                        <tr data-entry-id="{{ $ticketComment->id }}">
                            <td>
                            ...
                            <td>
                                @can('ticket_comment_show')

                                    <a href="{{ route('admin.ticket-comments.show', $ticketComment->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                                ...
                                @can('ticket_comment_edit')

                                        <a href="{{ route('admin.ticket-comments.edit', $ticketComment->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                                ...
                                @can('ticket_comment_delete')


                                        <form action="{{ route('admin.ticket-comments.destroy', $ticketComment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('ticket_comment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.ticket-comments.massDestroy') }}",
...
@endcan
  ...
  // let table = $('.datatable-TicketComment:not(.ajaxTable)').DataTable({ buttons: dtButtons })
