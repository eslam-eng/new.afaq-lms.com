...
@section('content')
@can('attendance_design_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.attendance-designs.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.attendanceDesign.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.attendanceDesign.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-AttendanceDesign">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($attendanceDesigns as $key => $attendanceDesign)
                        <tr data-entry-id="{{ $attendanceDesign->id }}">
                            <td>
                            ...
                            <td>
                                @can('attendance_design_show')
{{--                                    <a href="{{ route('admin.attendance-designs.show', $attendanceDesign->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">--}}
...
{{--                                    </a>--}}
                                ...
                                @can('attendance_design_edit')
                                        <a href="{{ route('admin.attendance-designs.edit', $attendanceDesign->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                                ...
                                @can('attendance_design_delete')
                                        <form action="{{ route('admin.attendance-designs.destroy', $attendanceDesign->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
