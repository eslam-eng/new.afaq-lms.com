    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tickets.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="ticket_category_id" id="ticket_category_id" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('replies_number') ? 'is-invalid' : '' }}" type="number" name="replies_number" id="replies_number" value="{{ old('replies_number', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('statues') ? 'is-invalid' : '' }}" name="statues" id="statues">
                    ...
                    @foreach(App\Models\Ticket::STATUES_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('statues', '') === (string) $key ? 'selected' : '' }}>
                            {{ trans('cruds.ticket.fields.'.$label)  }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('image') ? 'is-invalid' : '' }}" accept="image/png, image/gif, image/jpeg" id="image-dropzone">
                </div>
                @if($errors->has('image'))
                    <div class="invalid-feedback">
                        {{ $errors->first('image') }}
