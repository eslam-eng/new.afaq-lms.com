...
@section('content')
@can('slider_card_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.slider-cards.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.sliderCard.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.list') }} {{ trans('cruds.sliderCard.title_singular') }} 
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-SliderCard">
                <thead>
                    <tr>
<th></th>
                        <th>
                ...
                <tbody>
                    @foreach($sliderCards as $key => $sliderCard)
                    <tr data-entry-id="{{ $sliderCard->id }}">
<td></td>
                        <td>
                            {{ $sliderCard->id ?? '' }}
                        </td>
                        ...
                        <td>
                            @can('slider_card_show')

                                <a href="{{ route('admin.slider-cards.show', $sliderCard->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('slider_card_edit')

                                    <a href="{{ route('admin.slider-cards.edit', $sliderCard->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('slider_card_delete')

                                    <form action="{{ route('admin.slider-cards.destroy', $sliderCard->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
