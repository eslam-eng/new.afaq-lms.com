...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.cancelRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cancel-requests.update", [$cancelRequest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('approved') ? 'is-invalid' : '' }}" name="approved" id="approved">
                    ...
                    @foreach(App\Models\CancelRequest::APPROVED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('approved', $cancelRequest->approved) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
