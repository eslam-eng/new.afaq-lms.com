...
@section('content')
    @can('business_need_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.business-needs.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.businessNeed.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.businessNeed.title_singular') }} {{ trans('global.list') }}
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-BusinessNeed">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach ($businessNeeds as $key => $businessNeed)
                            <tr data-entry-id="{{ $businessNeed->id }}">
                                <td>
                                ...
                                <td>
                                    @if ($businessNeed->icon)
                                        <a href="{{ $businessNeed->icon->getUrl() }}" target="_blank"
                                ...
                                <td>
                                    @can('business_need_show')
                                        <a href="{{ route('admin.business-needs.show', $businessNeed->id) }}" type="button"
                                    ...
                                    @can('business_need_edit')
                                        <a href="{{ route('admin.business-needs.edit', $businessNeed->id) }}" type="button"
                                    ...
                                    @can('business_need_delete')
                                        <form action="{{ route('admin.business-needs.destroy', $businessNeed->id) }}"
        ...
        $(function() {
            ...
            @can('business_need_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('admin.business-needs.massDestroy') }}",
            ...
            // let table = $('.datatable-BusinessNeed:not(.ajaxTable)').DataTable({ buttons: dtButtons })
