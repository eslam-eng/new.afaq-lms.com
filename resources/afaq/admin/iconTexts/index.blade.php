...
@section('content')
@can('icon_text_create')
<div style="margin-bottom: 10px;" class="row">
    <div class="col-lg-12">
        <a class="btn btn-success" href="{{ route('admin.icon-texts.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.iconText.title_singular') }}
        </a>
    </div>
</div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('global.list') }} {{ trans('cruds.iconText.title_singular') }} 
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-IconText">
                <thead>
                    <tr>
<th></th>
                        <th>
                ...
                <tbody>
                    @foreach($iconTexts as $key => $iconText)
                    <tr data-entry-id="{{ $iconText->id }}">
<td></td>
                        <td>
                            {{ $iconText->id ?? '' }}
                        </td>
                        ...
                        <td>
                            @can('icon_text_show')
                                ...
                                <a href="{{ route('admin.icon-texts.show', $iconText->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light">
                            ...
                            @can('icon_text_edit')
                                    ...
                                    <a  href="{{ route('admin.icon-texts.edit', $iconText->id) }}" type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light">
                            ...
                            @can('icon_text_delete')

                                    <form action="{{ route('admin.icon-texts.destroy', $iconText->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
