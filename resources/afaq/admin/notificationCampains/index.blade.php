...
@section('content')
    @can('notification_campain_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.notification-campains.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.notificationCampain.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.notificationCampain.title_singular') }} {{ trans('global.list') }}
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-NotificationCampain">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <th>
                                {{ trans('cruds.notificationCampain.fields.title') }}
                            </th>
                            <th>
                                {{ trans('cruds.notificationCampain.fields.message') }}
                            </th>
                    ...
                    <tbody>
                        @foreach ($notificationCampains as $key => $notificationCampain)
                            <tr data-entry-id="{{ $notificationCampain->id }}">
                                <td>
                                ...
                                <td>
                                    {{ app()->getLocale() == 'en' ? $notificationCampain->specialty->name_en ?? '' : $notificationCampain->specialty->name_ar ?? '' }}
                                </td>

                                <td>
                                    {{ app()->getLocale() == 'en' ? $notificationCampain->title_en ?? '' : $notificationCampain->title_ar ?? '' }}
                                </td>

                                <td>
                                    {{ app()->getLocale() == 'en' ? $notificationCampain->message_en ?? '' : $notificationCampain->message_ar ?? '' }}
                                ...
                                <td>
                                   {{date('h:i:s a m/d/Y', strtotime($notificationCampain->send_at)) ?? ''}}
                                </td>
                                <td>
                                    {{ App\Models\NotificationCampain::STATUS_RADIO[$notificationCampain->status] ?? '' }}
                                </td>
                                <td>
                                    @can('notification_campain_show')
                                        <a href="{{ route('admin.notification-campains.show', $notificationCampain->id) }}"
                                            type="button"
                                    ...
                                    @can('notification_campain_edit')
                                        @if ($notificationCampain->status == 0)
                                            <a href="{{ route('admin.notification-campains.edit', $notificationCampain->id) }}"
                                                type="button"
                                    ...
                                    @can('notification_campain_delete')
                                        @if ($notificationCampain->status == 0)
                                            <form
                                                action="{{ route('admin.notification-campains.destroy', $notificationCampain->id) }}"
        ...
        $(function() {
            ...
            @can('notification_campain_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('admin.notification-campains.massDestroy') }}",
