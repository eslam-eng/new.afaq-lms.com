...
@section('content')
    @can('course_quize_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.course-quizes.create',['course_id' => $course_id]) }}">
                    {{ trans('global.add') }} {{ trans('cruds.courseQuize.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="card">
        <div class="card-header">
            {{ trans('global.list') }} {{ trans('cruds.courseQuize.title_singular') }} 
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-CourseQuize">
                    <thead>
                    <tr>
                        <th></th>
                        <th>
                    ...
                    <tbody>
                    @foreach($courseQuizes as $key => $courseQuize)
                        <tr data-entry-id="{{ $courseQuize->id }}">
                            <td></td>
                            <td>
                                {{ $courseQuize->id ?? '' }}
                            </td>
                            ...
                            <td>
                                {{app()->getLocale()=='en' ?  $courseQuize->course->name_en ?? '' :  $courseQuize->course->name_ar ?? ''}}
                            </td>
                            <td>
                                {{app()->getLocale()=='en' ?  $courseQuize->exam_title->name_en ?? '' :  $courseQuize->exam_title->name_ar ?? ''}}

                            </td>
                            <td>
                                {{app()->getLocale()=='en' ?  $courseQuize->title_en ?? '' :  $courseQuize->title_ar ?? ''}}

                            </td>

                            <td>
                                {{app()->getLocale()=='en' ?  $courseQuize->description_en ?? '' :  $courseQuize->description_ar ?? ''}}
                            ...
                            <td>
                                @can('course_quize_show')
                                    <a type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light" href="{{ route('admin.course-quizes.show',['course_id' => $course_id ,'id' => $courseQuize->id]) }}">
                                    ...
                                    <a type="button" class="btn btn-icon btn-icon rounded-circle btn-primary waves-effect waves-float waves-light" href="{{ route('admin.course-quizes.results',['course_id' => $course_id ,'id' => $courseQuize->id]) }}">
                                        <i class="fas fa-poll"></i>
                                    </a>

                                @endcan

                                @can('course_quize_edit')
                                    <a  type="button" class="btn btn-icon btn-icon rounded-circle btn-warning waves-effect waves-float waves-light" href="{{ route('admin.course-quizes.edit', ['course_id' => $course_id ,'id' => $courseQuize->id]) }}">
                                ...
                                @can('course_quize_delete')
                                    <form action="{{ route('admin.course-quizes.delete',['course_id' => $course_id ,'id' => $courseQuize->id]) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@section('scripts')
    ...
    <script>
        ...
        //         url: "{{ route('admin.course-quizes.massDestroy') }}",
        ...
        //         pageLength: 100,
        //     });
        //     let table = $('.datatable-CourseQuize:not(.ajaxTable)').DataTable({ buttons: dtButtons })
